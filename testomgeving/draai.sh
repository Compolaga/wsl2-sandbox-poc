...
mkdir -p "$POC/evidence"

BOOT='set -e
cp -r /poc /home/dev/poc && chmod -R u+w /home/dev/poc
mkdir -p /home/dev/.claude
cp -r /cfg/. /home/dev/.claude/ 2>/dev/null || true
case "'"$POLICY"'" in
  slice1) cp /home/dev/poc/config/settings.slice1.json /home/dev/.claude/settings.json ;;
  slice2) sudo mkdir -p /etc/claude-code
          sudo cp /home/dev/poc/config/managed-settings.linux.json /etc/claude-code/managed-settings.json ;;
esac
echo "== omgeving =="
...
./run.sh '"$ARG"'
rc=$?
cp -r /home/dev/poc/evidence/. /evidence/ 2>/dev/null || true
exit $rc'

if [ "$MODE" = "shell" ]; then
  exec docker run --rm -it \
    -v "$POC:/poc:ro" -v "$CFG:/cfg:ro" -v "$POC/evidence:/evidence" \
    sandbox-poc bash -lc 'cp -r /poc /home/dev/poc && chmod -R u+w /home/dev/poc && mkdir -p /home/dev/.claude && cp -r /cfg/. /home/dev/.claude/ && cd /home/dev/poc && exec bash'
fi

docker run --rm \
  -v "$POC:/poc:ro" -v "$CFG:/cfg:ro" -v "$POC/evidence:/evidence" \
  sandbox-poc bash -lc "$BOOT"
