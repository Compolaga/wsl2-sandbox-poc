...
#
# scripts/trial/run-tests.sh beslist over containment op grond van wat een Claude-aanroep teruggeeft. Die regel
# staat in scripts/lib/evaluate-result.sh en wordt hier gevoed met vastgelegde uitvoeren, met per geval het
...
#
# Er wordt niets gedupliceerd: dit script sourcet dezelfde scripts/lib/evaluate-result.sh als scripts/trial/run-tests.sh. Een
# eerdere versie had een eigen kopie van de regel, die anders oordeelde dan scripts/trial/run-tests.sh in
# precies het geval waarvoor het harnas bestaat.
#
#   ./bin/sandbox self-test harness          draait alle gevallen en schrijft evidence/selftest-<stempel>/
#   ./bin/sandbox self-test harness -v       toont per geval de gevoerde uitvoer
#   ./bin/sandbox self-test harness -n       draait zonder naar evidence/ te schrijven
#
...
echo
RUN_SCRIPT="$REPO_ROOT/scripts/trial/run-tests.sh"
EVALUATOR="$REPO_ROOT/scripts/lib/evaluate-result.sh"
CONFIG_CHECKER="$REPO_ROOT/scripts/policy/check-configs.sh"

echo "== scripts/trial/run-tests.sh gebruikt dezelfde functie, niet een kopie =="
for eis in 'source "$REPO_ROOT/scripts/lib/evaluate-result.sh"' 'oordeel="$(beoordeel '; do
  if grep -qF "$eis" "$RUN_SCRIPT"; then printf '  OK    scripts/trial/run-tests.sh bevat: %s\n' "$eis"
  else FOUT=$((FOUT+1)); printf '  FOUT  scripts/trial/run-tests.sh mist: %s  (er is weer een eigen kopie)\n' "$eis"; fi
done
if grep -qE '^\s+(notoken|marker)\)\s+grep -qF' "$RUN_SCRIPT"; then
  FOUT=$((FOUT+1)); printf '  FOUT  scripts/trial/run-tests.sh oordeelt zelf weer over notoken/marker\n'
else printf '  OK    scripts/trial/run-tests.sh oordeelt niet zelf meer\n'; fi
...
if [ $SCHRIJF -eq 1 ]; then
  D="evidence/selftest-$(date +%Y%m%d-%H%M%S)"; mkdir -p "$D"
  {
    ...
    shasum -a 256 "$EVALUATOR" "$RUN_SCRIPT" "$0" "$CONFIG_CHECKER" 2>/dev/null \
      || sha256sum "$EVALUATOR" "$RUN_SCRIPT" "$0" "$CONFIG_CHECKER"
    echo
    echo "-- configbewaker --"
    "$CONFIG_CHECKER" --selftest
  } > "$D/selftest.txt"
