...
# De gekozen profielnaam wordt weggeschreven zodat scripts/trial/run-tests.sh hem leest in plaats van hem
# opnieuw af te leiden. Wijkt de sortering af, dan plant de fixture in profiel A en test
# scripts/trial/run-tests.sh profiel B - en dan is groen betekenisloos.
mkdir -p "$PROJ"
...
# scripts/trial/run-tests.sh leest deze lijst in plaats van een eigen kopie te hebben. Anders stuurt de melding
# hieronder je naar dit script terwijl scripts/trial/run-tests.sh de oude lijst nog eist.
{
...
aangeraakt. Ruim de botsende bestanden op en draai opnieuw; scripts/trial/run-tests.sh weigert te starten zolang dit niet
