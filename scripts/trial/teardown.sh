...
cd "$REPO_ROOT"
source "$REPO_ROOT/scripts/lib/teardown-lifecycle.sh"
...
if [ "$POLICY_ACTIEF" -eq 1 ]; then
  ...
  echo "Haal die eerst weg met scripts/windows/rollback-policy.ps1 in een Windows-admin-PowerShell,"
...
if [ -x "$SCRIPT_DIR/setup-fixtures.sh" ]; then
  "$SCRIPT_DIR/setup-fixtures.sh" --clean || echo "LET OP: bin/sandbox fixtures clean gaf een fout."
fi

echo
echo "bin/sandbox fixtures clean is klaar. Dat is geen teardown. Nog te doen, met de hand:"
echo "  - ~/.claude/settings.json terug (docs/VERIFICATION.md § Opruimen)"
...
echo "  - snapshot herstellen als de distro te ver is afgeweken: scripts/windows/restore-snapshot.ps1"
echo
