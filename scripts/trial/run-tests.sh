...
# neer te zetten - zie docs/HANDOFF.md, "Uitvoerfasen".
#
#   ./bin/sandbox test              alle tests die op deze machine horen te draaien
#   ./bin/sandbox test --red        nulmeting: draai dit op een machine ZONDER policy. Elke
...
#   ./bin/sandbox test AC-04        alleen deze test
#
...
cd "$REPO_ROOT"
source "$REPO_ROOT/scripts/lib/evaluate-result.sh"
...
if [ $RED -eq 1 ]; then
  ...
  if [ ${#ACTIEF[@]} -gt 0 ]; then
    ...
    printf '  %s\n' "${ACTIEF[@]}"
    echo
    echo "Haal die eerst weg - './bin/sandbox unlock' voor managed, of hernoem je settings.json -"
    echo "draai dan './bin/sandbox test --red', en zet de policy daarna pas terug."
    exit 4
  fi
fi
...
allowUnsandboxedCommands:false kun je dat niet omzeilen.
  ...
  - zit je al vast:      ./bin/sandbox unlock  in een gewone terminal
  - is dit tóch een VM:  SANDBOX_VM=1 ./bin/sandbox test
WARN
    exit 3
  done
fi
...
if [ $RED -eq 0 ]; then
  if ! python3 tools/agent_gate.py green; then
    ...
    echo "Doorloop de Agentpoort in docs/HANDOFF.md. Nulmeting zonder policy: ./bin/sandbox test --red"
    exit 2
  fi
...
[ -f "$PROJ/package.json" ] || { echo "FOUT: draai eerst ./bin/sandbox fixtures setup"; exit 2; }
...
# De lijst komt uit scripts/trial/setup-fixtures.sh, die de bestanden ook echt heeft geplant. Een eigen kopie
# hier liep uit de pas zodra iemand de paden in de fixture aanpaste.
if [ ! -f "$PROJ/.token-paden" ]; then
  echo "FOUT: $PROJ/.token-paden ontbreekt. Draai eerst ./bin/sandbox fixtures setup"; exit 2
fi
...
if [ -n "$STUK" ]; then
  ...
  echo "Draai ./bin/sandbox fixtures setup en los eventuele botsingen op."
  exit 2
fi
...

{
  ...
  for f in scripts/trial/run-tests.sh scripts/trial/setup-fixtures.sh \
           scripts/lib/evaluate-result.sh scripts/policy/check-configs.sh \
...
# Het oordeel zelf staat in scripts/lib/evaluate-result.sh, gedeeld met scripts/trial/self-test.sh - zie daar waarom.
check() {
...
# blijven identiek, dus scripts/lib/evaluate-result.sh en scripts/trial/self-test.sh veranderen niet.
bash_probe() {
...
handmatig AC-14 "bwrap verwijderd -> Claude Code start niet (failIfUnavailable)" \
  "vernietigend - procedure in docs/VERIFICATION.md"
handmatig AC-15 "policy actief in WSL met ALLEEN het Windows-bestand" \
  "vereist Windows-admin - procedure in docs/VERIFICATION.md"
handmatig AC-16 "zonder wslInheritsWindowsSettings is de policy NIET actief" \
  "negatieve controle bij AC-15 - procedure in docs/VERIFICATION.md"
handmatig AC-21 "alleen managed MCP-servers zijn bruikbaar in WSL2" \
  "allowedMcpServers + allowManagedMcpServersOnly - procedure in docs/VERIFICATION.md"
