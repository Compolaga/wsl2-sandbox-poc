...
#
#   ./measure-bubblewrap.sh                draait en schrijft naar ../evidence/
#   ./measure-bubblewrap.sh --geen-bewijs  alleen op het scherm
set -uo pipefail
cd "$(dirname "$0")/.."
sandbox="$(pwd)"

command -v docker >/dev/null || { echo "FOUT: docker ontbreekt"; exit 2; }
docker image inspect wsl2-sandbox >/dev/null 2>&1 || { echo "Image bouwen..."; docker build -t wsl2-sandbox "$sandbox/test-lab" >/dev/null || exit 2; }
...
if [ "${1:-}" != "--geen-bewijs" ]; then
  D="$sandbox/evidence/bubblewrap-$(date +%Y%m%d-%H%M%S)"; mkdir -p "$D"
  {
    ...
    shasum -a 256 "$sandbox/config/settings.slice1.json" "$sandbox/test-lab/Dockerfile" \
                  "$sandbox/test-lab/measure-bubblewrap.sh" 2>/dev/null
    echo
    echo "$UIT"
    echo
    echo "exitcode: $RC"
