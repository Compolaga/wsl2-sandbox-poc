...
# samen met de token op de VM, zodat de meting nooit ongemerkt een oude suite gebruikt.
#
...
#
# Gebruik:
#   ./round-2-measurement.sh ~/.sandbox-token            # meten, VM laten staan
#   ./round-2-measurement.sh ~/.sandbox-token --afbreken # meten en daarna de resourcegroep slopen
#   ./round-2-measurement.sh ~/.sandbox-token "" AC-00p  # alleen één AC meten (diagnostiek)
#
...
# Altijd de huidige lokale suite meten; evidence en test-lab horen niet in het pakket.
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
tar czf "$WERK/pakket.tgz" -C "$ROOT" \
  bin/sandbox \
  scripts/lib/repo-root.sh scripts/lib/evaluate-result.sh \
  scripts/policy/agent-gate.sh scripts/policy/check-configs.sh \
  scripts/trial/setup-fixtures.sh scripts/trial/run-tests.sh scripts/trial/self-test.sh \
  config \
...
find /home/dev/repo/bin /home/dev/repo/scripts -type f -name '*.sh' -exec chmod +x {} +
...
sudo -u dev env CLAUDE_CODE_OAUTH_TOKEN="$TOK" HOME=/home/dev bash -lc '
  cd ~/repo
  ./bin/sandbox fixtures setup > /mnt/c/sandbox/fixture.out 2>&1
  FILTER="$(tr -d " \r\n" < /mnt/c/sandbox/filter)"
  if [ -n "$FILTER" ]; then
    ./bin/sandbox test "$FILTER" > /mnt/c/sandbox/run.out 2>&1
  else
    ./bin/sandbox test           > /mnt/c/sandbox/run.out 2>&1
  fi
