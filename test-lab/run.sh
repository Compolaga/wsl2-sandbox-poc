...
#
#   ./test-lab/run.sh nulmeting    geen policy: alles moet lekken (bewijst dat de tests kunnen falen)
#   ./test-lab/run.sh slice1       user settings
#   ./test-lab/run.sh slice2       managed settings in /etc/claude-code (de echte grens)
#   ./test-lab/run.sh shell        interactieve shell in de container
#
...
echo
cd /home/dev/repo
./bin/sandbox fixtures setup >/dev/null 2>&1 || { echo "fixture faalde"; ./bin/sandbox fixtures setup; exit 2; }
set +e
./bin/sandbox test '"$ARG"'
rc=$?
