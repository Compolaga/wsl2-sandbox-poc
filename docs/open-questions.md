...
Eén vraag staat open: OQ-8. OQ-5, OQ-6 en OQ-7 zijn opgelost. Twee zijn niet beantwoord
...
heeft. **OQ-6 is opgelost:** ITOps accepteert de gemeten Read-werking; AC-23 blijft die per
laptop controleren. OQ-7 moet op een nieuwe doellaptop als regressiecontrole worden
herhaald, maar de onderzoeksvraag zelf is beantwoord.
...
| OQ-2 | Welke toolchains gebruikt het doelteam? | **Uitgesteld** tot de implementatie. Eigenaar: ITOps. Luc gaf 21-08 aan: bijna allemaal, proef doet alleen Node. `allowRead` en `allowedDomains` zijn daarop teruggebracht. |
...
| OQ-4 | Welke Claude Code-versie rolt de organisatie uit? | **Beantwoord** door Luc, 21-08: latest. Of elke gebruikte key in die versie zit is niet nagetrokken — A10 in de handoff draagt ITOps op dat per key in de documentatie te doen. |
...
| OQ-6 | Kan een developer interactief goedkeuring geven voor lezen buiten de werkdirectory? | **Opgelost 23-08-2026.** Ja; ITOps accepteert die werking. AC-23 blijft de handmatige controle en ieder gevoelig pad moet expliciet in `permissions.deny` staan. |
...
Een **lege array** betekent: geen enkele server toegestaan. Wil de organisatie bepaalde servers wél,
...
## OQ-6 — Read-werking geaccepteerd [OPGELOST 23-08-2026]
...
Daarom is AC-23 geen containment-test maar een handmatige constatering. **ITOps accepteert
deze werking; Luc bevestigde dat besluit op 23-08-2026.** Er is geen aanvullende vaste
`defaultMode` of blokkade buiten de werkdirectory nodig. De blijvende gedragsregel is dat
ieder gevoelig pad letterlijk in `permissions.deny` moet staan. De inventaris uit OQ-1
blijft daardoor een uitrolvoorwaarde.
...
preflight in `bin/sandbox test` slaat er niet op aan, want die kijkt alleen naar een brede `denyRead`.

**Verifieer het vóór je de config installeert, niet erna.** Zet het bestand neer en draai als
eerste:

```bash
./bin/sandbox test AC-20
```
...
`./bin/sandbox unlock` — dan is de lock strenger dan gedacht en hoort er een expliciete `allowRead` in
de managed config.
