...
`bin/sandbox test` schrijft een ingevulde kopie naar `evidence/<stempel>/proof-matrix.md`. Dit bestand
...
| Windows → WSL | Gekozen Windows-mappen staan in Linux, niet als symlink naar `/mnt` | | `bin/sandbox workspace` |
| `bin/sandbox policy validate` payload | Locks en `_beschermd` overleefden de merge | | |
| `bin/sandbox self-test` | Het harnas kan falen | | *geen* sandbox-bewijs |
| `bin/sandbox test --red` | Containmentproeven lekken zonder policy | | |
| `bin/sandbox test` groen | Canary komt niet terug; toegestane paden wel | | |
...
| docs/VERIFICATION.md | Twaalf interactieve controles in een gewone sessie | | |

## Nog niet vrijgegeven — dit blijft open

- [ ] Tweede **developer-laptop** (niet de laptop waarop deze run draaide) heeft dezelfde matrix groen, inclusief AC-16 en docs/VERIFICATION.md.
...
- [ ] Fixtures opgeruimd (`docs/VERIFICATION.md` § Opruimen).
